...
namespace {
const char* kLeaderboardFile = "leaderboard.txt";
...
std::vector<ScoreEntry> LoadLeaderboard()
{
    ...
    std::ifstream fin(kLeaderboardFile);
    if (!fin.is_open()) return board;
    ...
    while (std::getline(fin, line))
    {
        ...
        try {
            int s = std::stoi(line.substr(0, pos));
            std::string ts = line.substr(pos + 1);
            board.push_back({ s, ts });
        }
        catch (...) {
            // ignore bad line
        }
...
void SaveLeaderboard(const std::vector<ScoreEntry>& board)
{
    std::ofstream fout(kLeaderboardFile, std::ios::trunc);
...
std::vector<ScoreEntry> UpdateLeaderboard(int new_score)
{
    auto board = LoadLeaderboard();
    board.push_back({ new_score, NowTimeString() });
...
int BestScore(const std::vector<ScoreEntry>& board)
{
    return board.empty() ? 0 : board.front().score;
}
} // namespace
...
int Controller::GameOver()//游戏结束界面
{
    ...
    auto board = UpdateLeaderboard(score);

    int best_score = BestScore(board);
    ...
    std::cout << " ┃                                          ┃";
    Sleep(30);
    // Top3 显示（仅显示分数，避免界面拥挤）

    int top1 = board.size() > 0 ? board[0].score : 0;

    int top2 = board.size() > 1 ? board[1].score : 0;

    int top3 = board.size() > 2 ? board[2].score : 0;

    SetCursorPosition(12, 16);

    std::cout << "Top1:" << top1 << "  Top2:" << top2;
